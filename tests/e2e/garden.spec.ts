...
test('garden map: bed creation, assignment, tap-through, drag, unassign', async ({
  ...
  await page.getByRole('button', { name: 'Increase height' }).click()
  await page.getByRole('button', { name: 'Save' }).click()
  const tile = page.locator('article').filter({ hasText: 'South bed' })
  await expect(tile).toBeVisible()
  const sized = await tile.boundingBox()
  expect(Math.round(sized!.width)).toBe(100)
  expect(Math.round(sized!.height)).toBe(80)
